    ...
    <div class="container-fluid">
      ...
      <div class="row">
        ...
        >
          ...
          <ul class="nav" v-show="ishas_search==true">
            <li
          ...
          <ul class="nav" v-show="ishas_search==false">
            <li class="nav-item">未找到</li>
          </ul>
          <div class="row" style="height:2rem"></div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    getSearch() {
      this.$axios
        .post("/api/getSearch", {
          search_text: this.search_List
        })
        .then(response => {
          if (response.data.length) {
            this.search_list = response.data;
            this.ishas_search = true;
          } else {
            this.search_list = response.data;
            this.ishas_search = false;
          }
        });
    },
    toUserHome(username) {
